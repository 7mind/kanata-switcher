...
- GNOME top-bar extension now preserves last known status while daemon owner is unavailable and runs a periodic owner probe to refresh status/pause state after suspend/resume DBus hiccups, instead of staying on disconnected `?` state indefinitely.
