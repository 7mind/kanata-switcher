...
27. **macOS accept() inherits O_NONBLOCK from listener** - unlike Linux (where `accept()` always returns a blocking socket regardless of listener flags), macOS inherits O_NONBLOCK. The mock Kanata server in integration tests uses a non-blocking listener; `set_nonblocking(false)` must be called on the accepted stream explicitly, otherwise `BufReader::read_line()` returns `WouldBlock` immediately and the server drops the connection before reading client handshake messages.
...
54. **macOS backend uses NSWorkspace notifications** - Uses `NSWorkspaceDidActivateApplicationNotification` via `objc2` bindings. The notification callback extracts `NSRunningApplication` info (localizedName, bundleIdentifier) and sends it over a channel to the tokio event loop. `CFRunLoopRef` is stored as `usize` to satisfy `Send` bound. Focus query on startup/unpause uses `current_window_info()` directly (no DBus).
55. **Windows backend uses WinEvent hook** - Uses `SetWinEventHook(EVENT_SYSTEM_FOREGROUND)` via `windows-sys` crate. A dedicated thread runs a `GetMessage` pump to dispatch events. The event callback extracts `pid` → `GetWindowModuleFileNameW` (class) + `GetWindowTextW` (title) and sends over a channel. Focus query on startup/unpause reads foreground window directly.
56. **Platform dispatch is compile-time via cfg** - `#[cfg(target_os = "linux")]` / `#[cfg(target_os = "macos")]` / `#[cfg(target_os = "windows")]` selects the correct `detect_environment_impl()` and `platform::run()`. No runtime probing needed on non-Linux. The `platform/` module encapsulates all OS-specific entry points (signal handling, supervisor setup, etc.) so `main.rs` has only one `#[cfg]` block.
57. **Cargo.toml uses platform-conditional deps** - `zbus` (Linux), `wayland-client` (Linux), `x11rb` (Linux), `objc2` + `objc2-foundation` + `objc2-application` (macOS), `windows-sys` (Windows) are all conditional on `cfg(target_os = ...)`.
58. **Tests are cfg-gated too** - All integration tests that depend on Linux-only types (`zbus`, `wayland`, `x11rb`) are behind `#[cfg(target_os = "linux")]`. The macOS/Windows backends have no integration tests yet (require running kanata).
59. **build.rs skips glib-schemas on non-Linux** - The GNOME extension schema compilation step is skipped on macOS/Windows.
60. **Window class/title normalization is shared** - `normalize_process_name()` and `current_window_info_static()` are used across platforms for the focus query path. macOS and Windows implement their own normalization if needed (currently not needed since NSWorkspace and WinEvent return clean strings).
