...
# Notes
- 2026-03-16: Continuous lifecycle mode now falls back to generic Wayland when runtime target resolution fails for a Wayland snapshot (both snapshot-triggered and periodic capability recheck paths), instead of keeping stale backend/idle state indefinitely. Added regression `test_run_lifecycle_supervisor_continuous_wayland_resolver_error_falls_back_to_generic_wayland`.
