...
# Notes
- 2026-03-23: NixOS/Home Manager module now supports optional multiplex mode via `services.kanata-switcher.keyboards` (one unit per keyboard: `kanata-switcher-<name>`), while preserving the previous single-instance top-level mode. Added module assertions for mode exclusivity and per-instance `configFile`/`settings` mutual exclusivity, plus QA checklist `qa/nix-module-keyboards-checklist.md`.
- 2026-03-19: Fixed GNOME lock/unlock indicator regression in focus-only mode. `selectStatus()` now falls back to `lastStatus` when focus status is missing or has an empty/invalid layer, preventing top-bar `?` while daemon state is valid. Added gjs regressions in `tests/gnome-extension-format.js`.
- 2026-03-18: Fixed GNOME suspend/resume indicator regression where temporary DBus owner loss could pin top-bar label to `?`. Extension now preserves last known status across disconnect and starts a periodic daemon-owner reconnect probe to refresh `GetStatus`/`GetPaused` after bus hiccups even when `g-name-owner` notify recovery is missed. Added gjs regression coverage in `tests/gnome-extension-daemon-state.js`.
- 2026-03-18: Fixed logind `User.Display` signal decode regression that caused fail-fast exits during GNOME <-> Linux console transitions. `decode_logind_display_path_change` now accepts structure-wrapped object paths (including variant-wrapped structures), matching real `PropertiesChanged` signal encodings. Added regressions `test_decode_logind_display_path_change_parses_variant_wrapped_structure` and `test_decode_logind_display_path_change_parses_variant_wrapped_structure_empty_path`.
