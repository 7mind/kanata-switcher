...
#[tokio::test]
async fn test_resolve_logind_session_path_for_unknown_env_stops_after_retry_deadline() {
    with_test_timeout(async {
        let calls = Arc::new(AtomicUsize::new(0));
        let calls_clone = calls.clone();
        let result = resolve_logind_session_path_for_env_with_resolver(
            Environment::Unknown,
            &[0, 0],
            move || {
        ...
        assert!(result.is_err(), "retry deadline should return an error");
        let message = result.err().expect("error expected").to_string();
        assert!(
            message.contains("not ready before startup retry deadline"),
            "error should mention startup retry deadline exhaustion"
        );
        assert_eq!(
            calls.load(Ordering::SeqCst),
            3,
            "unknown-env retries should stop after configured delay budget"
        );
    })
    .await;
}
...
#[tokio::test]
async fn test_resolve_logind_session_path_for_unknown_env_retries_until_success() {
    with_test_timeout(async {
        let calls = Arc::new(AtomicUsize::new(0));
        let calls_clone = calls.clone();
        let expected = OwnedObjectPath::try_from("/org/freedesktop/login1/session/c1")
            .expect("path parse should succeed");
        ...
        .await
        .expect("resolver should succeed before retry deadline");

        assert_eq!(result, expected);
        assert_eq!(
            calls.load(Ordering::SeqCst),
            4,
            "resolver should be retried until first success"
        );
    })
    .await;
}
...
#[tokio::test]
async fn test_resolve_logind_session_path_for_non_unknown_env_does_not_retry() {
    with_test_timeout(async {
        let calls = Arc::new(AtomicUsize::new(0));
        let calls_clone = calls.clone();
        let result = resolve_logind_session_path_for_env_with_resolver(
        ...
        assert!(result.is_err(), "non-unknown env should error immediately");
        let message = result.err().expect("error expected").to_string();
        assert!(
            message.contains("logind user has no display session"),
            "non-unknown env should keep immediate display-not-ready failure behavior"
        );
        assert_eq!(
            calls.load(Ordering::SeqCst),
            1,
            "non-unknown env should not retry display-not-ready resolution"
        );
    })
    .await;
}
