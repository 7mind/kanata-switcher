...
async fn test_dbus_service_layer_switching() {
    ...
    // Verify layer change was sent
    let msg = server.recv_timeout(Duration::from_secs(2));
    assert_eq!(msg, Some(KanataMessage::ChangeLayer { new: "terminal".to_string() }));
}
...
async fn test_dbus_service_virtual_keys() {
    ...
    assert_eq!(msg1, Some(KanataMessage::ChangeLayer { new: "browser".to_string() }));

    let msg2 = server.recv_timeout(Duration::from_secs(2));
    assert_eq!(msg2, Some(KanataMessage::ActOnFakeKey {
    ...
    assert_eq!(msg3, Some(KanataMessage::ActOnFakeKey {
        name: "vk_browser".to_string(),
        action: "Release".to_string(),
    }));

    let msg4 = server.recv_timeout(Duration::from_secs(2));
    assert_eq!(msg4, Some(KanataMessage::ChangeLayer { new: "default".to_string() }));
}
...
async fn test_dbus_service_fallthrough() {
    ...
    let msg3 = server.recv_timeout(Duration::from_secs(2));
    assert_eq!(msg3, Some(KanataMessage::ChangeLayer { new: "terminal".to_string() }));
}
...
impl DbusSessionGuard {
    fn start() -> Result<Self, String> {
        ...
        let config_content = format!(r#"<!DOCTYPE busconfig PUBLIC "-//freedesktop//DTD D-Bus Bus Configuration 1.0//EN" "http://www.freedesktop.org/standards/dbus/1.0/busconfig.dtd">
<busconfig>
  <type>session</type>
  ...
  <policy context="default">
    ...
    <allow own="*"/>
        ...
        let mut child = std::process::Command::new("dbus-daemon")
            .args(["--config-file", config_path.to_str().unwrap(), "--nofork", "--print-address"])
        ...
        if address.is_empty() {
            ...
            if let Some(mut stderr) = child.stderr.take() {
                ...
                let _ = child.kill();
                return Err(format!("dbus-daemon produced no address. stderr: {}", err_output));
            }
            let _ = child.kill();
...
impl XvfbGuard {
    ...
    fn start(display_num: u32) -> Option<Self> {
        ...
        wait_for(|| {
            x11rb::connect(Some(&display_clone)).ok()
        }).ok()?;
...
fn test_x11_property_notify() {
    ...
    daemon_conn.change_window_attributes(
        root,
    ...
    let event = wait_for(|| {
        daemon_conn.poll_for_event().ok().flatten()
    }).expect("Timeout waiting for PropertyNotify event");

    match Some(event) {
        Some(x11rb::protocol::Event::PropertyNotify(e)) => {
            assert_eq!(e.atom, atoms._NET_ACTIVE_WINDOW, "Expected _NET_ACTIVE_WINDOW property change");
            ...
            assert!(prop_reply.value.len() >= 4, "Expected window ID in property");
...
fn test_x11_focus_handler_integration() {
    ...
    app_conn.create_window(
        ...
        0, 0, 100, 100, 0,
        WindowClass::INPUT_OUTPUT, 0, &CreateWindowAux::default()
    ).unwrap();
    ...
    WrapperExt::change_property32(
        ...
        x11_state.atoms._NET_ACTIVE_WINDOW, AtomEnum::WINDOW,
        &[win]
    ).unwrap();
    ...
    assert!(actions.actions.contains(&FocusAction::ChangeLayer("test-layer".to_string())));
}
...
fn test_x11_multiple_focus_changes() {
    ...
    let x11_state = X11State { connection: conn, root, atoms };
    ...
    app_conn.create_window(
        x11rb::COPY_DEPTH_FROM_PARENT, win1, root,
        0, 0, 100, 100, 0,
        WindowClass::INPUT_OUTPUT, 0, &CreateWindowAux::default()
    ).unwrap();
    WrapperExt::change_property8(
        &app_conn,
        PropMode::REPLACE, win1,
        AtomEnum::WM_CLASS, AtomEnum::STRING,
        b"instance\0App1\0"
    ).unwrap();
    ...
    app_conn.create_window(
        x11rb::COPY_DEPTH_FROM_PARENT, win2, root,
        0, 0, 100, 100, 0,
        WindowClass::INPUT_OUTPUT, 0, &CreateWindowAux::default()
    ).unwrap();
    WrapperExt::change_property8(
        &app_conn,
        PropMode::REPLACE, win2,
        AtomEnum::WM_CLASS, AtomEnum::STRING,
        b"instance\0App2\0"
    ).unwrap();
    ...
    assert!(actions.actions.contains(&FocusAction::ChangeLayer("layer1".to_string())));
    ...
    let actions = handler.handle(&info, "default").unwrap();
    assert!(actions.actions.contains(&FocusAction::ChangeLayer("layer2".to_string())));
